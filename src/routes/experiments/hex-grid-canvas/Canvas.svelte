<script lang="ts">
	...
	const selectHex = (event: CustomEvent<Direction>) => {
		...
		switch (event.detail) {
			case 'Up':
				newCoordinates = {
					...
					s: selectedHex.coordinates.s + 1,
					r: selectedHex.coordinates.r - 1
				}
				break
			case 'UpRight':
				newCoordinates = {
					...selectedHex.coordinates,
					q: selectedHex.coordinates.q + 1,
					r: selectedHex.coordinates.r - 1
				}
				break
			case 'DownRight':
		...
		context.setTransform(
			...
			0,
			1,
			selectedHex.canvasCoordinates.x,
			selectedHex.canvasCoordinates.y
		)
	...
	const drawGrid = () => {
		...
		for (let y = 0; y < height + radius; y += radius * Math.sin(angle)) {
			...
			for (
				...
				console.log(
					`cubed q:${cubedCoordinates.q}, r:${cubedCoordinates.r}, s:${cubedCoordinates.s}`
				)
...
<div class="root-layout">
	<div class="control-area">
		...
		<InfoArea {selectedHex} />
	</div>
	<canvas {width} {height} style:backgroundColor bind:this={canvas} />
</div>
